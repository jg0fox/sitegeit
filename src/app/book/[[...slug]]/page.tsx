...
function BookingPageContent() {
  ...
  const bookingSlug = params.slug?.[0] || null
  const refBusinessId = searchParams.get('ref')
  ...
  const loadDays = useCallback(async () => {
    ...
    const slugParam = bookingSlug ? `&slug=${bookingSlug}` : ''
    const res = await fetch(`/api/bookings/slots?month=${monthStr}${slugParam}`)
    if (res.ok) {
  ...
  const loadSlots = useCallback(async (date: string) => {
    ...
    const slugParam = bookingSlug ? `&slug=${bookingSlug}` : ''
    const res = await fetch(`/api/bookings/slots?date=${date}${slugParam}`)
    if (res.ok) {
      const data = await res.json()
      setSlots(data.slots || [])
    }
